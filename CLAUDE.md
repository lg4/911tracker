...
- **Category filters (T8/T14/T29/T31/T34):** Police/Fire/EMS chips (restyled as accented icon+text CSS buttons in T34; Civil folded under Police so only these three buckets remain — `classify()` returns `'police'` by default with no fourth bucket) + per-category colored heat layers, time-window buttons, classification extracted into unit-tested `web/classify.js` (T31 widened the EMS numeric-code pattern — feed codes use a third-char alphabet of A/B/C/D/O, not just B/C/D). Per-subtype detail views shipped (T17); heat-only below max zoom with per-point category icons exclusively at z14 (T30/T32; T28's always-on recent markers were removed in T32). Point popups mirror Oneida County's CAD record (T35): Sequence No., Created date/time rendered in America/New_York matching the county report verbatim, Status, Type, Location — all entity-escaped before `bindPopup`.
- **Basemap (T9/T10/T33 done):** local vendored OSM tile pyramid z7–z14 full-county plus a selective z15 deepening around Utica and Rome only (~15km windows each; rest of the county tops out at z14 and renders blank past it, noWrap) plus CSS dark-mode variant of the same tiles (no second pyramid, keyless/offline); regenerate via `scripts/gen-tiles.js`. Map/tile-layer maxZoom + MAX_ZOOM = 15 together (icons hand over at z15 per T32). Uncovered margins outside the pyramid render per-theme — near-black in dark mode (clings to the dark tiles), pale blue-grey in light (`body.light` override).
